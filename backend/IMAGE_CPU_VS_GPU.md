...
- **Windows:** Run **`setup_windows.bat`**. It installs base deps and `requirements-extra.txt`, then if **NVIDIA is detected** (`nvidia-smi`) it installs **PyTorch with CUDA** automatically (tries **cu130** for 13.x driver, then cu128, cu124, cu121). No manual pip step needed.
- **Linux/macOS:** Run **`./setup_fullstack.sh`**. It installs `requirements-extra.txt`, then if **NVIDIA is detected** it installs PyTorch with CUDA (cu130 → cu128 → cu124 → cu121). On Mac (no NVIDIA) image gen uses CPU unless you use Metal builds separately.

**CUDA 13.x driver:** PyTorch provides **cu130** (CUDA 13.0); use it if you have a 13.0/13.1 driver. There is no **cu131** wheel; cu130 works with 13.1 (driver is backward compatible).
...
- **Manual fix** (re-run setup or install manually)  
  ...
  Re-run the setup script, or in the backend venv: `pip install torch --index-url https://download.pytorch.org/whl/cu130` (or cu128 / cu124 / cu121). Restart the backend after installing.
